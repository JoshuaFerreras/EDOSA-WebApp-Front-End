        ...
        <div class="contents">
            <div class="view-patient-form">
                ...
                <form @submit.prevent>
                    ...
                    <div class="form-group" v-if="profile_form?.info?.height">
                        <label for="height">Height(cm):</label>
                        <input type="number" id="height" v-model="profile_form.info.height" :disabled="isDisabled">
                    </div>
                    <div class="form-group" v-if="profile_form?.info?.weight">
                        <label for="weight">Weight(kg):</label>
                        <input type="number" id="weight" v-model="profile_form.info.weight" :disabled="isDisabled">
                    </div>
                    <div class="form-group" v-if="profile_form?.info?.birthdate">
                        <label for="birthdate">Birthdate:</label>
                        <input type="text" id="birthdate" v-model="profile_form.info.birthdate" :disabled="isDisabled">
                    </div>
                    <div class="form-group" v-if="profile_form?.info?.address">
                        <label for="address">Address:</label>
                        <textarea id="address" v-model="profile_form.info.address" :disabled="isDisabled"></textarea>
                    </div>
                    <div class="form-group" v-if="profile_form?.info?.civil_status">
                        <label for="civilstatus">Civil Status:</label>
                        <input type="text" id="civilstatus" v-model="profile_form.info.civil_status" :disabled="isDisabled">
                    </div>
                    <div class="form-group" v-if="profile_form?.info?.mobile_number">
                        <label for="mobile-number">Mobile Number:</label>
                        <input type="text" id="mobile-number"  placeholder="09XXXXXXXXX" v-model="profile_form.info.mobile_number" :disabled="isDisabled">
                    </div>
                    <div class="form-group" v-if="profile_form?.info?.contact_person">
                        <label for="emergency-number">Emergency Contact Person:</label>
                        <input type="text" id="mobile-number" placeholder="Optional" v-model="profile_form.info.contact_person" :disabled="isDisabled">
                    </div>
                    <footer>
                        <div class="buttons">
                            <RouterLink to="/patient-dashboard" class="button" style="background-color: whitesmoke; border: 2px solid #11499C; color: #11499C;"><i class="fa-solid fa-backward"></i> Go Back</RouterLink>
                            ...
                            <div v-if="isDisabled === true">
                                <button type="button" @click="editProfile" style="background-color: red; color: white;" class="button">Edit Profile <i class="fa-solid fa-pen-to-square"></i></button>
                            </div>
                            <div v-else>
                                <button type="button" @click="cancel" style="background-color: red; color: white;" class="button">Cancel  <i class="fa-solid fa-pen-to-square"></i></button>
                            </div>
                            <div v-if="isDisabled === false && profile_form?.info?.last_name == null">
                                <button type="submit" @click="submitPatientInfo" style="background-color: #11499C; color: white;" class="button">Save Changes <i class="fa-solid fa-check"></i></button>
                            </div>
                            <div v-if="isDisabled === false && profile_form?.info?.last_name != null">
                                <button type="submit" @click="updatePatientInfo(profile_form.info.id)" style="background-color: #11499C; color: white;" class="button">Save Changes <i class="fa-solid fa-check"></i></button>
                            </div>
                        </div>
                    </footer>
                </form>
